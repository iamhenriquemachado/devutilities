...
export function CsvToJsonFormatter() {
  ...
  const formatCsv = async () => {
    ...
    try {
      ...
      try {
        ...
        const response = await fetch("http://localhost:8000/api/csv-to-json", {
          method: "POST",
          headers: {
        ...
        if (!response.ok) {
          throw new Error(data.error || "Failed to convert CSV to JSON")
        }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-4">
        <Button onClick={formatCsv} disabled={!input || loading} className="bg-jam-purple hover:bg-jam-darkPurple">
          {loading ? (
            <>
